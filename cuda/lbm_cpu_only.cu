...
// Compile as:
// nvcc -o lbm_cpu_only lbm_cpu_only.cu -std=c++11
...
int main(int argc, char** argv)
{
    ...
    // Compute disparity map
		auto start_time_kernel = std::chrono::system_clock::now();   
	...
	auto end_time_kernel = std::chrono::system_clock::now();   
	auto program_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_time_kernel  - start_time_kernel).count();
	cout << "Disparity map generation time: " << program_time << "ms" << endl;
	...
	program_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count();
	cout << "Total execution time: " << program_time << "ms" << endl;
  output_path = output_dir + "/disparity_cpu.png"; 	
