...
int main(int argc, char** argv)
{
	...
	int disparity_min = 0, disparity_max = 70;
	int disparity_range = disparity_max - disparity_min;
	int half_block_size = 4;
...
//	minMaxLoc(disp_img, &minVal, &maxVal); // Find min and max pixel values in the input image
//	Mat outputImage;

// Scale the pixel values to fit the full dynamic range (0-255)
//	disp_img.convertTo(outputImage, CV_8U, 255.0 / (maxVal - minVal), -minVal * 255.0 / (maxVal - minVal));
	for(int i=0; i<ROW; i++)
	{
		for(int j=0; j<COL; j++)
		{
